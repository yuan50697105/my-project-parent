...
public class GoodsInfoController {
    @Autowired
    private BGoodsInfoService customerInfoService;

    @RequestMapping(value = "data", method = {RequestMethod.GET})
    public Result<?> data(TbGoodsInfoQuery query,
                          @RequestParam(defaultValue = "1") int page,
                          @RequestParam(defaultValue = "20") int size) {
        PageResult<TbGoodsInfo> pageResult = customerInfoService.selectByQuery(query, page, size);
        return ResultInfo.success(pageResult.getData(), pageResult.getTotalRows());
    }

    @RequestMapping(value = "list", method = {RequestMethod.GET})
    public Result<?> list(TbGoodsInfoQuery query) {
        List<TbGoodsInfo> list = customerInfoService.selectByQuery(query);
